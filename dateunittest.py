...
class TestDateMethods(unittest.TestCase):
    ...
    def test_init(self):
        ...
        length = TestDateMethods.lengthOfYear(now.year)
    ...
    def test_init_raise(self):
        ...
        for month in range(1, datetime.date.max.month + 1):
            ...
            with self.assertRaises(ValueError):
                d = date.Date(month, 0, now.year)
            lengthOfThisMonth = TestDateMethods.lengthOfMonth(month, now.year)
            with self.assertRaises(ValueError):
                d = date.Date(month, lengthOfThisMonth + 1, now.year)

    @staticmethod
    def lengthOfYear(year):
